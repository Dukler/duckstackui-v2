...
export default function ReactVirtualizedTableDEMO({id,slice,style,children,columns,actions}) {
    const router = useRouter();
    const value = useSelector(state => state[slice]?.byId?.genderId?.source)
    const state = {value};
    useEffect(()=>{
        console.log("demostate",state)
    },[state])
